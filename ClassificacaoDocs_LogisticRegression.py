...
#Configure Logging
#logging.basicConfig(filename="LogisticRegression_ClassificacaoDocs.log", format='%(asctime)s %(message)s', filemode='w') 
...
def process_model (X_train, X_test, Y_train, Y_test):
    ...
    vectorizer = CountVectorizer(analyzer="word",tokenizer=None,preprocessor=None,stop_words=PTBRStopsWords)
    logistReg = LogisticRegression()
    pipeline = Pipeline([('vect', vectorizer), ('logreg', logistReg)])
    pipeline.fit(X_train, Y_train)
    pipeline_score = pipeline.score(X_test, Y_test)
    print("\n-----------------------------------------------")
    print("\nPipeline_final_Score: {}".format(pipeline_score))
    print("\n-----------------------------------------------")
    return pipeline, logistReg, vectorizer
...
def evaluate_model(model, vectorizer, X_train, X_test, Y_test):
    ...
    print("\n-----------------------------------------------")
    print("\nLogistic Regression - Accuracy_Score: {}".format(accuracy_score))
    print("\n-----------------------------------------------")
